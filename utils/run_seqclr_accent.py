...
from seqclr.modules.dtw import window_mapping_and_dtw, window_mapping_and_syllable, window_mapping_and_character
...
if __name__ == "__main__":
    ...
    for ep in trange(epochs):
        ...
        for iter, batch in enumerate(dl_train): # 1-batch
            ...
            # START_END = torch.tensor(np.array(START_END))
            # START_END = START_END.T
            SEGMENTS = torch.tensor(np.array(SEGMENTS))
            SEGMENTS = SEGMENTS.T

            SYLLABLE = torch.tensor(np.array(SYLLABLE))
            SYLLABLE = SYLLABLE.T
                ...
                for i in range(num_samples):
                    for j in range(num_samples):
                        if i!=j:
                            ...
                            if "dtw" in config.model_outdir: 
                                mapped_zi, mapped_zj = window_mapping_and_dtw(zi, zj, wp, SEGMENTS[i].T, SEGMENTS[j].T)
                            elif "syllable" in config.model_outdir:
                                mapped_zi, mapped_zj = window_mapping_and_syllable(zi, zj, SYLLABLE[i].T, SYLLABLE[j].T)  
                            elif "character" in config.model_outdir:
                                mapped_zi, mapped_zj = window_mapping_and_character(zi, zj, SEGMENTS[i].T, SEGMENTS[j].T)
